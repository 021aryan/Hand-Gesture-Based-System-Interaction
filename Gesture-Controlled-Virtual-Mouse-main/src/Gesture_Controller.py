...
class Controller:
    ...
    def handle_controls(gesture, hand_result):  
        ...
        print(gesture)
        x,y = None,None
        if gesture == Gest.ROCK and not Controller.rockflag:# 9=rock
            Controller.rockflag=True
            pyautogui.keyDown("alt")
        if Controller.rockflag == True and gesture == Gest.FIST:
                pyautogui.press("tab")
                Controller.rockflag=False
        if gesture == Gest.THREE:                          # 14=Three Fingers
            pyautogui.keyUp("alt")
        if gesture ==6 and not Controller.lockflag:
            Controller.lockflag=True
        if gesture==8 and Controller.lockflag:
            ctypes.windll.user32.LockWorkStation()
            Controller.lockflag=False
...
class GestureController:
    ...
    def __init__(self):
        ...
        GestureController.CAM_HEIGHT = GestureController.cap.set(cv2.CAP_PROP_FRAME_HEIGHT,600)
        GestureController.CAM_WIDTH = GestureController.cap.set(cv2.CAP_PROP_FRAME_WIDTH,600)
    ...
    def start(self):
        ...
        with mp_hands.Hands(max_num_hands = 2,min_detection_confidence=0.5, min_tracking_confidence=0.5) as hands:
            while GestureController.cap.isOpened() and GestureController.gc_mode:
                ...
                cv2.moveWindow('Gesture Controller', 600, 100)
